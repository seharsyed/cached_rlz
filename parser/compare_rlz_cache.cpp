...
void print_usage(const char* prog) {
    std::cerr
        << "Usage:\n"
        << "  " << prog << " <reference> <suffix-array> <input> [bucket_size] [trials]\n\n"
        << "For your current DNA sanity run, use the same file as reference and input:\n"
...
int main(int argc, char** argv) {
    ...
    const std::size_t bucket_size = argc >= 5 ? static_cast<std::size_t>(std::stoull(argv[4])) : 256;
    const int trials = argc >= 6 ? std::stoi(argv[5]) : 3;

    if (bucket_size == 0) {
        std::cerr << "bucket_size must be > 0\n";
        return 1;
    }
    if (trials <= 0) {
    ...
    for (int t = 1; t <= trials; ++t) {
        ...
        std::cout << "  baseline trial " << t << ": " << std::fixed << std::setprecision(3) << ms << " ms\n";
    }

    CachedRLZParser<Symbol, SAType> parser(ref, sa, bucket_size);
    Triples cached;
    ...
    for (int t = 1; t <= trials; ++t) {
        ...
        std::cout << "  cached trial " << t << ":   " << std::fixed << std::setprecision(3) << ms
                  << " ms  hits=" << final_info.hits
                  << " misses=" << final_info.misses
                  << " entries=" << final_info.current_size << "\n";
    }

    const bool equal = (baseline == cached);
    std::cout << "baseline factors   : " << base.size() << "\n";
    std::cout << "cached factors     : " << cached.size() << "\n";

    std::cout << "\n=== RESULT ===\n";
    std::cout << "outputs equal       : " << (equal ? "YES" : "NO") << "\n";
    std::cout << "phrases             : " << baseline.size() << "\n";
    std::cout << "baseline best ms    : " << std::fixed << std::setprecision(3) << baseline_best << "\n";
    std::cout << "cached best ms      : " << std::fixed << std::setprecision(3) << cached_best << "\n";
    std::cout << "speedup             : " << std::fixed << std::setprecision(3)
              << (cached_best > 0.0 ? baseline_best / cached_best : 0.0) << "x\n";
    std::cout << "cache hits          : " << final_info.hits << "\n";
    std::cout << "cache misses        : " << final_info.misses << "\n";
    std::cout << "cache hit rate      : " << std::fixed << std::setprecision(6) << final_info.hit_rate << "\n";
    ...
    std::cout << "cache load factor   : " << std::fixed << std::setprecision(3) << final_info.load_factor << "\n";
    std::cout << "approx cache MB     : " << std::fixed << std::setprecision(3)
              << final_info.approx_bytes / (1024.0 * 1024.0) << "\n";
    std::cout << "peak RSS MB         : " << std::fixed << std::setprecision(2)
              << peak_rss_kb() / 1024.0 << "\n";
